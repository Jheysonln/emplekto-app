// src/presentation/components/features/auth/register-form.tsx
'use client';
...
import { RegisterFormData, registerSchema } from '@/presentation/validators/auth.schemas';
import { ROUTES } from '@/shared/constants/app.constants';


/**
 * Formulario de registro premium con diseño moderno
 */
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          <label className="block text-sm font-semibold text-gray-700 mb-4">
            ¿Cómo quieres usar EmplekTo?
          </label>
          <div className="grid grid-cols-2 gap-4">
            <label className={`relative cursor-pointer group ${watchedRole === 'JobSeeker' ? 'ring-2 ring-blue-500' : ''}`}>
              <input
                {...register('roleName')}
                type="radio"
                value="JobSeeker"
                className="sr-only"
              />
              <div className={`p-4 rounded-xl border-2 transition-all duration-300 text-center hover:shadow-lg ${
                watchedRole === 'JobSeeker' 
                  ? 'border-blue-500 bg-blue-50' 
                  : 'border-gray-200 bg-white/50 hover:border-gray-300'
              }`}>
                <Briefcase className={`mx-auto mb-2 ${
                  watchedRole === 'JobSeeker' ? 'text-blue-600' : 'text-gray-400'
                }`} size={24} />
                <span className={`text-sm font-semibold ${
                  watchedRole === 'JobSeeker' ? 'text-blue-700' : 'text-gray-600'
                }`}>
                  Busco Empleo
                </span>
              </div>
            ...
            <label className={`relative cursor-pointer group ${watchedRole === 'Employer' ? 'ring-2 ring-blue-500' : ''}`}>
              <input
                {...register('roleName')}
                type="radio"
                value="Employer"
              ...
              <div className={`p-4 rounded-xl border-2 transition-all duration-300 text-center hover:shadow-lg ${
                ...
                <Building className={`mx-auto mb-2 ${
                  watchedRole === 'Employer' ? 'text-blue-600' : 'text-gray-400'
                }`} size={24} />
                <span className={`text-sm font-semibold ${
                  watchedRole === 'Employer' ? 'text-blue-700' : 'text-gray-600'
                }`}>
                  Busco Talento
                </span>
              </div>
      ...
      >
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-4 bg-white/70 text-gray-500 font-medium">o regístrate con</span>
        </div>
      </motion.div>
